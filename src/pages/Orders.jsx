...
const OrderCard = ({ id, date, total, status, items, address, onTrack, onCancel, isDashboard }) => {
  const statusColors = {
      ...
      <div className="flex flex-col md:flex-row justify-between gap-6">
        <div className="space-y-4 flex-1">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className={`text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full border ${statusColors[status] || statusColors.Processing}`}>{status}</span>
          </div>
            ...
            <div className="flex -space-x-3 overflow-hidden">
              {items && items.map((item, i) => (
                <div key={i} className="w-12 h-12 rounded-xl border-2 border-white bg-slate-50 overflow-hidden shadow-sm">
                  <img src={typeof item === 'string' ? 'https://images.unsplash.com/photo-1584308666744-24d5c474f2ae?q=80&w=100' : item.image} alt="" className="w-full h-full object-cover" />
                </div>
              ))}
            </div>
            <div className="min-w-0">
               <p className="text-sm font-black text-dark truncate">
                 {items && (typeof items[0] === 'string' ? items[0] : items[0].name)} 
                 {items && items.length > 1 ? ` & ${items.length - 1} more` : ''}
               </p>
        ...
        <div className="md:w-64 space-y-4 border-t md:border-t-0 md:border-l border-slate-100 pt-6 md:pt-0 md:pl-8">
           <div className="flex justify-between items-center">
              ...
              <span className="text-xl font-black text-primary tracking-tighter">Rs. {total}</span>
           </div>
           {address && (
           ...
           <div className="grid grid-cols-1 gap-2">
            ...
            {status !== 'Delivered' && status !== 'cancelled' && (
              <button 
                onClick={onCancel}
                className="w-full py-3 text-[9px] font-black text-rose-500 hover:bg-rose-50 rounded-xl uppercase tracking-widest transition-all"
              >
                Cancel Order
              </button>
            )}
...
const Orders = () => {
  const { orders, cancelOrder } = useDashboard();
  ...
  const content = (
    <div className={`max-w-5xl mx-auto px-6 ${!isDashboard ? 'pt-32 pb-20' : 'py-6'}`}>
      <div className="space-y-12">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
              ...
              <div className="flex items-center gap-3 bg-white p-2 rounded-2xl border border-slate-100 shadow-sm">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-black text-dark italic">{ongoingOrders.length} Shipments Live</p>
                  </div>
              </div>
            </div>
          )}
          ...
          <div className="space-y-8">
              ...
              {ongoingOrders.length > 0 ? (
                ...
                <div className="bg-white p-12 rounded-[3.5rem] border border-slate-100 text-center opacity-50">
                  <Package className="w-12 h-12 mx-auto mb-4 text-slate-300" />
                  <p className="text-sm font-bold italic text-slate-400">No active shipments found.</p>
                </div>
              )}
          </div>
          ...
          <div className="space-y-8 pt-6">
              ...
              {pastOrders.length > 0 ? (
                ...
                <div className="bg-white p-12 rounded-[3.5rem] border border-slate-100 text-center opacity-50">
                   <p className="text-sm font-bold italic text-slate-400">No past orders available.</p>
                </div>
              )}
          </div>
