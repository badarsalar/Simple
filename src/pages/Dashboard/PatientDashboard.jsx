      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
        ...
        <motion.section variants={itemVariants} className="space-y-6">
           <div className="flex items-center justify-between px-2">
              <h3 className="text-xl font-black text-dark italic uppercase leading-none border-l-4 border-primary pl-4">Next Appointment</h3>
              <Link to="/dashboard/appointments" className="text-[10px] font-black text-primary uppercase tracking-widest flex items-center gap-1 hover:underline underline-offset-4">View All <ChevronRight className="w-3 h-3" /></Link>
           </div>
           
           {nextApp ? (
             <div className="bg-white rounded-[3.5rem] p-8 border border-slate-100 shadow-lg relative overflow-hidden group">
                ...
                <div className="flex items-center gap-6 relative z-10">
                   <div className="w-20 h-20 rounded-[2rem] border-4 border-primary/20 p-1 shrink-0 overflow-hidden shadow-xl">
                      <img src={nextApp.image} className="w-full h-full object-cover rounded-[1.5rem]" alt="" />
                   </div>
                   <div className="flex-1 min-w-0">
                      <h4 className="text-lg lg:text-xl font-black text-dark italic leading-tight uppercase truncate">{nextApp.doctor}</h4>
                      <p className="text-[10px] font-black text-primary uppercase tracking-widest mt-1 italic">{nextApp.specialty}</p>
                      <div className="flex items-center gap-4 mt-3">
                         <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400">
                            <Clock className="w-3 h-3" /> {nextApp.date} @ {nextApp.time}
                         </div>
                      </div>
                   </div>
                </div>
                ...
                <div className="mt-8 flex items-center gap-4 relative z-10">
                   <Link to="/dashboard/appointments" className="flex-1 px-6 py-4 bg-primary text-white rounded-2xl text-[10px] font-black uppercase italic tracking-widest shadow-xl shadow-primary/20 hover:scale-[1.02] transition-all flex items-center justify-center gap-3 text-center">
                      <Video className="w-4 h-4" /> Start Video
                   </Link>
                   <Link to="/dashboard/appointments" className="flex-1 px-6 py-4 bg-slate-50 border border-slate-100 text-dark rounded-2xl text-[10px] font-black uppercase italic tracking-widest hover:bg-slate-100 transition-all flex items-center justify-center gap-3 text-center">
                      <Clock className="w-4 h-4" /> Reschedule
                   </Link>
                </div>
             </div>
           ) : (
             <div className="bg-slate-50 rounded-[3.5rem] p-12 text-center border-2 border-dashed border-slate-200">
                <Calendar className="w-12 h-12 text-slate-300 mx-auto mb-4" />
                <p className="text-sm font-black text-slate-400 italic uppercase">No upcoming appointments</p>
                <Link to="/doctors" className="text-primary text-[10px] font-black uppercase italic mt-4 block underline">Book Now</Link>
             </div>
           )}
        </motion.section>
        ...
        <motion.section variants={itemVariants} className="space-y-6">
           <div className="flex items-center justify-between px-2">
              <h3 className="text-xl font-black text-dark italic uppercase leading-none border-l-4 border-amber-500 pl-4">Delivery Status</h3>
              <Link to="/dashboard/orders" className="text-[10px] font-black text-primary uppercase tracking-widest flex items-center gap-1 hover:underline underline-offset-4">Order History <ChevronRight className="w-3 h-3" /></Link>
           </div>
           
           {orders[0] ? (
             <div className="bg-dark rounded-[3.5rem] p-8 text-white shadow-2xl relative overflow-hidden group">
                <div className="absolute bottom-0 left-0 w-48 h-48 bg-primary/10 rounded-full blur-3xl -translate-x-1/2 translate-y-1/2"></div>
                
                <div className="flex justify-between items-start mb-10 relative z-10">
                   <div>
                      <span className="text-[10px] font-black text-primary uppercase tracking-widest bg-primary/10 px-3 py-1 rounded-full border border-primary/20 italic">{orders[0].id}</span>
                      <h4 className="text-2xl font-black italic mt-3 flex items-center gap-3 uppercase">Order <span className="text-primary italic">{orders[0].status}</span></h4>
                   </div>
                   <div className="text-right">
                      <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Total Value</p>
                      <p className="text-xl font-black text-white italic tracking-tighter mt-1">${orders[0].total}</p>
                   </div>
                </div>

                <div className="space-y-6 relative z-10">
                   <div className="flex items-center gap-3">
                      <Package className="w-10 h-10 text-primary p-2 bg-primary/10 rounded-xl" />
                      <div>
                         <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Items</p>
                         <p className="text-xs font-bold text-white/80 italic mt-1">{orders[0].items.join(' + ')}</p>
                      </div>
                   </div>
                   ...
                   <div className="relative pt-4">
                      ...
                      <div className="mt-4 flex justify-between text-[10px] font-black uppercase italic tracking-widest">
             ...
             <div className="bg-slate-50 rounded-[3.5rem] p-12 text-center border-2 border-dashed border-slate-200">
                ...
                <p className="text-sm font-black text-slate-400 italic uppercase">No active orders</p>
             </div>
           )}
        </motion.section>
      </div>
      ...
      >
         ...
         <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-10">
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto">
               <Link to="/doctors" className="flex-1 px-10 py-5 bg-primary text-white rounded-2xl font-black text-xs uppercase italic tracking-widest shadow-xl shadow-primary/20 hover:scale-105 transition-all text-center flex items-center justify-center gap-3">
                  <Calendar className="w-5 h-5" /> Book Appointment
               </Link>
               <Link to="/dashboard/vault" className="flex-1 px-10 py-5 bg-white/10 backdrop-blur-md text-white rounded-2xl font-black text-xs uppercase italic tracking-widest hover:bg-white/20 transition-all text-center flex items-center justify-center gap-3 border border-white/10">
